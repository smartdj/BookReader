...
//                            $model = new ChuangShiBook_TableOfContents;
//                            $model->book_id = $bookId;
//                            $model->juan_title = $juanTitle;
//                            $model->chapter_title = $chapterElem->find("b",0)->innertext;
//                            $model->chapter_url = $chapterElem->href;
//                            $model->update_time = $chapterElem->title;
//                            if(preg_match("/\d+-\d+-\d+ \d+:\d+:\d+/", $model->update_time, $matches)) {
//                                $model->update_time = $matches[0];
//                            }
//                            if($model->save() == false)
//                                return;
//                        }
                        //使用事务插入数据避免频繁写入导致缓慢
                        Db::transaction(function() use($chapterElems, $bookId, $juanTitle){
                            foreach ($chapterElems as $chapterElem){
                                $data = array();
                                $data["book_id"] = $bookId;
